...
def summarize_chunk(model, tokenizer, intro: str, chunk: str, word_limit: int) -> str:
    messages = [
        {"role": "system", "content": "You are a helpful assistant that summarizes interviews without reasoning."},
        {"role": "user", "content": (
            f"{intro}\n\nTranscript:\n{chunk}\n\n"
            f"Summarize key points, discussions, and speaker contributions. Provide summary in the same language as the input. Means if input language is Dutch the summary should be in Dutch, for German it should be in German and so on for other languages as well. Also, there should be one title of summary and one or two paragraph summary."
...
def summarize_interview(args):
    ...
    if raw_text_tokens <= token_threshold:
        ...
        try:
            summary = summarize_chunk(model, tokenizer, raw_text, args.summary_words)
...
def main():
    ...
    parser.add_argument("--summary-words-chunk", type=int, default=1500, help="Target word count *per chunk*")
    parser.add_argument("--intro-prompt", type=str, help="Short context: interviewer, interviewee, date, topic, etc.")
    parser.add_argument("--summary-words", type=int, default=500, help="Target word count for the total summary")
